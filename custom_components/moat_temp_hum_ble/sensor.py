...
Note that this file MUST be named sensor.py to match the top level name in
configuration.yaml.
"""
import logging
...
from .const import (
    CONF_DEVICE_MAC,
    CONF_DEVICE_NAME,
    CONF_DEVICE_CALIBRATE_TEMP,
    CONF_DEVICE_CALIBRATE_HUMIDITY,
    CONF_REPORT_FAHRENHEIT,
    DEFAULT_REPORT_FAHRENHEIT,
    CONF_ROUNDING,
    DEFAULT_ROUNDING,
    DEFAULT_DECIMALS,
    CONF_DECIMALS,
    ...
    CONF_LOG_SPIKES,
    DEFAULT_LOG_SPIKES,
    CONF_UPDATE_WHEN_UNAVAILABLE,
    DEFAULT_UPDATE_WHEN_UNAVAILABLE,
    CONF_USE_MEDIAN,
    ...
    CONF_RSSI_ENTITIES,
    DEFAULT_RSSI_ENTITIES,
    CONF_NUM_SAMPLES_ENTITIES,
    DEFAULT_NUM_SAMPLES_ENTITIES,
    CONF_GOVEE_DEVICES,
    CONF_HCI_DEVICE,
    CONF_MOAT_DEVICES,
    DEFAULT_HCI_DEVICE,
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        vol.Optional(
            CONF_REPORT_FAHRENHEIT, default=DEFAULT_REPORT_FAHRENHEIT
        ): cv.boolean,
        ...
        vol.Optional(
            CONF_UPDATE_WHEN_UNAVAILABLE, default=DEFAULT_UPDATE_WHEN_UNAVAILABLE
        ): cv.boolean,
        ...
        vol.Optional(
            CONF_TEMPERATURE_ENTITIES, default=DEFAULT_TEMPERATURE_ENTITIES
        ): cv.boolean,
        vol.Optional(
            CONF_HUMIDITY_ENTITIES, default=DEFAULT_HUMIDITY_ENTITIES
        ): cv.boolean,
        vol.Optional(
            CONF_BATTERY_ENTITIES, default=DEFAULT_BATTERY_ENTITIES
        ): cv.boolean,
        vol.Optional(CONF_RSSI_ENTITIES, default=DEFAULT_RSSI_ENTITIES): cv.boolean,
        vol.Optional(
            CONF_NUM_SAMPLES_ENTITIES, default=DEFAULT_NUM_SAMPLES_ENTITIES
        ): cv.boolean,
...
def setup_platform(
    ...
    device_wrappers: List[
        SensorDeviceWrapper
    ] = []  # Data objects of configured devices
    ...
    def handle_meta_event(hci_packet: HCIPacket) -> None:
        ...
        This callback will be called by the HCISocketPoller thread started by the bleson
        library.
        """
        ...
        if hci_packet.subevent_code == EVT_LE_ADVERTISING_REPORT:
            ...
            for curr_wrapper in device_wrappers:
                # Only process packets from devices that were explicitly added to the HA
                # configuration.
                if BDAddress(curr_wrapper.sensorDevice.mac) == packet_bd_address:
                    _LOGGER.debug(
                        "Received raw_data for %s, len %d (data %d): %s",
                        packet_bd_address,
                        hci_packet.length,
                        hci_packet.length - 11,
                        hex_string(hci_packet.data),
                    )
                    # parse raw_data data
                    parsed_advertisement = parse_ble_advertisement(
                    ...
                    if parsed_advertisement.packet is not None:
                        curr_wrapper.sensorDevice.update(
                            parsed_advertisement.temperature,
                    ...
                    # Update RSSI (even if the advertisement didn't have measurement
                    # data)
                    curr_wrapper.sensorDevice.append_rssi(parsed_advertisement.rssi)

    def init_configured_devices() -> None:
        """
        Initialize tracking for all devices listed in our configuration and register
        their Entities to Home Assistant.
        """

        shared_dev_params = CreateDeviceParams(
            report_fahrenheit=config[CONF_REPORT_FAHRENHEIT],
    ...
    def init_wrapper(
        device_config, device_params: CreateDeviceParams, brand: DeviceBrand
    ):
        # Override the shared configuration with the specific configuration for this
        # device.
        mac = device_config[CONF_DEVICE_MAC]
        device_params.description = device_config.get(CONF_DEVICE_NAME, None)
        device_params.calibrate_temp = device_config.get(
            CONF_DEVICE_CALIBRATE_TEMP, DEFAULT_DEVICE_CALIBRATE_TEMP
        )
        device_params.calibrate_humidity = device_config.get(
            CONF_DEVICE_CALIBRATE_HUMIDITY, DEFAULT_DEVICE_CALIBRATE_HUMIDITY
        )
        new_device = SensorDevice(mac, device_params)
        # Initialize Home Assistant Entities
        name = device_config.get(
            CONF_DEVICE_NAME, mac
        )  # Use provided name, or fall-back to using the MAC.
        ha_entities: List[TempHumSensorEntity] = []
        if config[CONF_TEMPERATURE_ENTITIES]:
            ha_entities.append(
                TemperatureEntity(mac, name, device_params.report_fahrenheit)
            )
    ...
    def async_report_device_data() -> None:
        """
        Move the collected data from each SensorDevice to the Home Assistant entities
        ...
        for curr_wrapper in device_wrappers:
            ...
            _LOGGER.debug(
                "Last mfg data for %r: %r, count=%d",
                device.mac,
                device.last_raw_data,
                temp_num,
            )
            ...
            if config[CONF_TEMPERATURE_ENTITIES]:
                ...
                entities[curr_entity_index].device_state_attributes[
                    "mean"
                ] = temperature_mean
                entities[curr_entity_index].device_state_attributes[
                    "median"
                ] = temperature_median
                entities[curr_entity_index].state = (
                    temperature_median if use_median else temperature_mean
                )
                curr_entity_index += 1
            ...
            if config[CONF_HUMIDITY_ENTITIES]:
                ...
                entities[curr_entity_index].state = (
                    humidity_median if use_median else humidity_mean
                )
                curr_entity_index += 1
            ...
            for entity in entities:
                ...
                # Note that NumSamplesPerPeriodEntity will still be updated,
                # since that is always valid.
                if config[CONF_UPDATE_WHEN_UNAVAILABLE] or entity.state is not None:
                    ...
                    # due to no samples received this period.
                    if (device.battery_millivolts is not None) or hasattr(
                        dev_state_attrs, "battery mV"
                    ):
                    ...
                    #  WARNING (SyncWorker_9) [homeassistant.helpers.entity] Entity
                    #  sensor.deep_freezer_temp is incorrectly being triggered for
                    ...
                    if entity.enabled:
                        ...
                        _LOGGER.debug(
                            "Entity %s is disabled; skip update.", entity.entity_id
                        )
    ...
    def schedule_update_ble_loop() -> None:
        ...
        # (And in the meantime, the HCISocketPoller thread will make calls to our
        # handle_meta_event callback.)
    ...
    def update_ble_loop(now) -> None:
        """
        Update the Home Assistant Entities from our collected data and refresh the
        BLE scanning.
        This probably shouldn't run in the event loop due to start_scanning potentially
        blocking.
        """
    ...
    # XXX: only supports LinuxProvider implementation (other platforms don't have
    #   _handle_meta_event to monkey-patch).
    # XXX: only supports hci0 - hci9
    adapter: Adapter = get_provider().get_adapter(int(config[CONF_HCI_DEVICE][-1]))
    # Monkey-patch the BluetoothHCIAdapter.  Would be better if the Adapter interface
...
class TempHumSensorEntity(Entity):
    ...
    def __init__(
        self, mac: str, entity_name: str, device_name: str, unique_id_prefix: str
    ):
    ...
    def should_poll(self) -> bool:
        """
        Don't poll; we will call schedule_update_ha_state from within our periodic
        update_ble_loop.
        """
        return False
    ...
    def device_info(self) -> Optional[Dict[str, Any]]:
        return {
            "identifiers": {(sensor.DOMAIN, self._mac)},
    ...
    async def async_added_to_hass(self) -> None:
        ...
        _LOGGER.info(
            "Adding entity %s (%s): %s", self.entity_id, self.unique_id, self.name
        )

    async def async_will_remove_from_hass(self) -> None:
        """Run when entity will be removed from hass."""
        _LOGGER.info(
            "Removing entity %s (%s): %s", self.entity_id, self.unique_id, self.name
        )
...
class NumSamplesPerPeriodEntity(TempHumSensorEntity):
    """
    HomeAssistant Entity for tracking number of the number measurements made during a
    period of time.
    """
